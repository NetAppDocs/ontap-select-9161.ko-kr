...
REST는 Roy Fielding이 그의 박사 학위 논문에서 공식적으로 언급했습니다.  https://www.ics.uci.edu/~fielding/pubs/dissertation/top.htm["논문"] 2000년 UC 어바인에서 이 논문은 일련의 제약 조건을 통해 아키텍처 스타일을 정의하며, 이러한 제약 조건들이 모여 웹 기반 애플리케이션과 그 기반 프로토콜을 개선합니다. 제약 조건은 상태 비저장 통신 프로토콜을 사용하는 클라이언트/서버 아키텍처 기반의 RESTful 웹 서비스 애플리케이션을 구축합니다.
